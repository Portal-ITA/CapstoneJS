# CapstoneJS Changelog

CapstoneJS is maintained by [@JedWatson](https://github.com/JedWatson) and an amazing team of contributors. All contributions are given credit here except for Jed's.

As of version 4 we have moved our changelog to the [GitHub releases]( https://github.com/capstonejs/capstone/releases) page. See there for the latest release notes.
...
* fixed; issue when capstone is a dependency of a global package, thanks [Robert Jensen](https://github.com/erg0dic)
...
* added; new hooks in the app middleware setup are available, see [#1736](https://github.com/capstonejs/capstone/issues/1736) for documentation.
...
* fixed; docs links now point to capstone site
...
* added; new `utc` option for `Date` and `DateTime` field types, see [#1487](https://github.com/capstonejs/capstone/issues/1487) for more details.
...
If you are interested in being part of Capstone's development team and aren't in our Slack channel, ping @JedWatson to get an invite!
...
Also, our site and documentation have been split out into their own repository: [capstonejs/capstonejs-site](https://github.com/capstonejs/capstonejs-site). Please contribute any changes, additions and issues to that new repo going forward.

## v0.3.9 / 2015-05-17

* added; Babel `object-assign` plugin for the Admin UI browserify build process
* improved; The whole Admin UI JS build system has been rewritten and should now be completely stable and much faster than any previous setup. You can enable development mode with the `CAPSTONE_DEV=true` environment variable
...
* improved; Now using [grappling-hook](https://github.com/capstonejs/grappling-hook), thanks [Camille Reynders](https://github.com/creynders)
...
* fixed; `Capstone.prototype.static(app)` was removed in 0.3.7, has been reintroduced for backwards-compatibility
...
Also in this release: Capstone is completely free of ESLint warnings thanks to the incredible efforts of [Camille Reynders](https://github.com/creynders) and [Jed Watson](https://github.com/JedWatson).
...
* fixed; refactored `doSignin()`, now exposed as `capstone.session.signinWithUser()`, thanks [Johnny Estilles](https://github.com/JohnnyEstilles)
...
* added; objects with IDs like `{ id: ObjectId }` can now be provided as Relationship values in data to `capstone.createItems(data, options, callback)`
* added; `options.refs` can be provided as an option to `capstone.createItems(data, options, callback)`
...
* updated; Many packages, see [e561fa6](https://github.com/capstonejs/capstone/commit/e561fa6c32a059f847283e98e2ecc95255829056)
...
* changed; `capstone.connect` was broken in 0.3.0, it has been replaced by the `capstone.set('express', ...)` and `capstone.set('mongoose', ...)` options
...
This is a major new version of CapstoneJS. The changes are too many to simply enumerate here, however we've done our best to ensure compatibility and feature parity with 0.2.x. If you experience any unexpected issues or behaviours, please let us know by opening a [Github Issue](https://github.com/capstonejs/capstone/issues).

For notes regarding breaking changes in the release, please see our [0.2.x to 0.3.x Changes wiki page](https://github.com/capstonejs/capstone/wiki/0.2.x-to-0.3.x-Changes).

Thanks to [all our contributors](https://github.com/capstonejs/capstone/graphs/contributors) for the huge amount of effort that went into this release, and the ongoing work designing, programming, testing and documenting CapstoneJS.

### Updated Express to 4.0

Express 4 included several breaking changes from Express 3. Capstone simplifies most of these changes, and if you're using `capstone.start()` to initialise your web server things will _probably just work_; however the more complicated your app, the more likely you will need to take these changes into account. Please review the [Moving to Express 4 Guide](http://expressjs.com/guide/migrating-4.html) for more information.
...
The UI has been cleaned up and improved, and the field types should support the same features and behaviours as in 0.2.x. If any field types or features don't continue to work as expected, please open a [GitHub Issue](https://github.com/capstonejs/capstone/issues) so we can fix it!
...
* added; `cors` middleware is now available as `Capstone.cors`
* fixed; redis session middleware doesn't support callbacks, should now initialise correctly
* fixed; issues parsing options in capstone.Email, thanks [Brett Newman](https://github.com/snowkeeper)
...
* fixed; `callback` is now correctly optional in `capstone.Email.send()`, thanks [Brett Newman](https://github.com/snowkeeper)
* improved; a `ReferenceError` is now thrown when an invalid list is requested with `capstone.list`, thanks [Sebastian McKenzie](https://github.com/sebmck)
...
* fixed; Capstone throws a warning when you add fields to a List that are implemented by the `track` option
...
* improved; redirects and error handlers are now the last items set up in `capstone.mount()`, thanks [Brett Newman](snowkeeper)
...
With this release, `connect-mongo` has been removed from Capstone's `package.json` and all three supported session store packages are optional. If you specify one, be sure to include it as a dependency in your project's `package.json` (Capstone will warn you on startup if you haven't).

Because of the way **npm** resolved paths, if you are using Capstone in development mode (via `npm link`) *and* using a session store, you have to `npm install <session-store>` in your local Capstone folder as well or it won't find the package.
...
* improved; Capstone now throws an error when no cookie secret is provided
...
* fixed; `capstone.createItems()` now creates items in series, not parallel
* added; support for dynamic queries for relationship values in `capstone.createItems()`
* added; verbose logging and strict ref checking options for `capstone.createItems()`
...
* fixed; Extracting and scoping `capstone.initAPI` to work correctly when used directly as middleware
...
* fixed; issues with Capstone.prototype.import, see [#348](https://github.com/capstonejs/capstone/issues/348), thanks [ashleycorker](https://github.com/ashleycoker)
* fixed; issues with geo handling in Location fields, see [#344](https://github.com/capstonejs/capstone/issues/344), thanks [mandb](https://github.com/mandb)

## v0.2.15 / 2014-05-13

* fixed; Added note to fields that didnt have one, thanks [Ötvös Richárd](https://github.com/RichardOtvos)
* fixed; Only show "Open Capstone" link to admins, thanks [John Beppu](https://github.com/beppu)
* fixed; Password fields are formatted correctly on the list screen of the Admin UI
* added; Support for custom MongoDB collection names (and other Schema options, see [#292](https://github.com/capstonejs/capstone/issues/292))
* added; Support for clearing password fields (if not required)
* added; Password.compare is now available on the Field object
* added; Support for loading fixture data with `capstone.createItems()` and in update scripts, see [this gist](https://gist.github.com/JedWatson/10739959) for an example
* added; Basic support for redirects, see [#303](https://github.com/capstonejs/capstone/issues/303) for details
...
* added; Warning when required fields aren't initial, see [#300](https://github.com/capstonejs/capstone/issues/300)
...
...as well as several other miscellaneous fixes and improvements, thanks to all our contributors who keep making Capstone better!
...
* added; `onHttpServerCreated` and `onHttpsServerCreated` events now fired during `capstone.start()`, allows for integration of things like socket.io
...
* added; `express` is now available as a property of `capstone`
...
* fixed; accented characters are converted correctly when generating slugs, thanks to [keystone-utils](https://github.com/capstonejs/keystone-utils) 0.1.7
...
* improved; ability to chain `capstone.pre`, `capstone.init`, `capstone.start`, `capstone.static`, `capstone.routes` and `capstone.bindEmailTestRoutes`
...
* added; https server and ssl configuration now supported by capstone.start(), thanks [snowkeeper](https://github.com/snowkeeper)
* improved; tweaks to native signin UI, thanks [jossmackison](https://github.com/JossMackison)

This version also contains the new docs and website developed by @jossmackison and @jedwatson. To view the docs locally, open `capstone/docs` and run `node docs`.
...
* added; new `capstone.import(path)` method for recusrively requiring all `.js` / `.coffee` files in a path relative to the project root, e.g. `capstone.import('models')`. Similar to but simpler than `capstone.importer()`.
...
* added; the http server is now accessible as `capstone.httpServer`, thanks [B. August](https://github.com/TheBenji)
...
* improved; much more flexible support for http server startup options, see [#154](https://github.com/capstonejs/capstone/issues/154)
...
CapstoneJS now requires mongoose 3.8.5+. Please test compatibility with your application before deploying this update to production.
...
* fixed; capstone error on blank date / date time fields, thanks [Mark Bayfield](https://github.com/mbayfield)
...
* added; current CapstoneJS version displayed in footer of Admin UI, see #130
...
* fixed; res.apiError was broken (when using `capstone.initAPI`)
...
* added; `capstone.content` class and the start of a page-content management framework
...
* added; support for callbacks in the `capstone.View` class. available callbacks are `err` (when the first argument returned by the query callback is not null, takes a single argument which is the error), `none` (when the results array is empty or the results argument is null), `then` (called unless `err` or `none` is present and called, takes three arguments: `err`, `results`, `next`)
...
* added; url parameter for the admin list view that updates all items in a list, optionally with data
	* `/capstone/list?update`
	* `/capstone/list?update={"field":"value"}`
