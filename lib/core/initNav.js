/**
 * Initialises Capstone's internal nav config
 *
 * @param {Object} nav
 * @api private
 */
...
module.exports = function initNav (sections) {
	...
	_.forEach(sections, function (section, key) {
		...
		section = {
			lists: section,
			label: nav.flat ? capstone.list(section[0]).label : utils.keyToLabel(key),
		};
		section.key = key;
		section.lists = _.map(section.lists, function (i) {
			if (typeof i === 'string') {
				var list = capstone.list(i);
				if (!list) {
					throw new Error('Invalid Capstone Option (nav): list ' + i + ' has not been defined.\n');
				}
				if (list.get('hidden')) {
					throw new Error('Invalid Capstone Option (nav): list ' + i + ' is hidden.\n');
				}
				...
				if (!_.has(i, 'key')) {
					throw new Error('Invalid Capstone Option (nav): object ' + i + ' requires a "key" property.\n');
				}
			...
			throw new Error('Invalid Capstone Option (nav): ' + i + ' is in an unrecognized format.\n');
		});
