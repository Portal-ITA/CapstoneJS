...
var debug = require('debug')('capstone:core:createItems');
...
function createItems (data, ops, callback) {
	...
	if (!_.isObject(data)) {
		throw new Error('capstone.createItems() requires a data object as the first argument.');
	}
	...
	function writeLog (data) {
		console.log(capstone.get('name') + ': ' + data);
	}
					...
					async.each(relationships, function (field, doneField) {
						...
						if (typeof fieldValue === 'function') {
							...
							var args = lists.map(function (i) {
								return capstone.list(i);
							});
							var query = fn.apply(capstone, args);
