...
function getHistoryModel (list, userModel) {
	...
	var schema = new capstone.mongoose.Schema({
		i: { type: capstone.mongoose.Schema.Types.ObjectId, ref: collection },
		t: { type: Date, index: true, required: true },
		o: { type: String, index: true, required: true },
		c: { type: [String], index: true },
		d: { type: capstone.mongoose.Schema.Types.Mixed, required: true },
	}, {
		id: true,
		versionKey: false,
	});
	...
	if (userModel) {
		schema.add({
			u: { type: capstone.mongoose.Schema.Types.ObjectId, ref: userModel },
		});
	}

	return capstone.mongoose.model(collection, schema, collection);
...
module.exports = function history () {
	...
	if (list.get('inherits')
		...
		&& capstone.mongoose.models[collectionName]) {
		console.log('List/model already exists for ' + collectionName + '.\nWon\'t re-create, capstone continuing.');
		return;
	}

	var userModel = capstone.get('user model');
