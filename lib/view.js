 ...
 * =================
 *
 * Helper to simplify view logic in a Capstone application
 *
 * @api public
 */
...
function View (req, res) {
	...
	if (!req || req.constructor.name !== 'IncomingMessage') {
		throw new Error('Capstone.View Error: Express request object is required.');
	}

	if (!res || res.constructor.name !== 'ServerResponse') {
		throw new Error('Capstone.View Error: Express response object is required.');
	}
 ...
 * Examples:
 *
 * view.query('books', capstone.list('Book').model.find());
 *
 ...
 * view.query(
 *     'admin.books',
 *      capstone.list('Book').model.find().where('user', 'Admin')
 * );
 *
		...
		query.exec(function (err, results) {
			...
			if (err) {
				if ('err' in callbacks) {
					 ...
					 *
					 * Example:
					 *     view.query('books', capstone.list('Book'))
				...
				if ((!results || (utils.isArray(results) && !results.length)) && 'none' in callbacks) {
					...
					if (utils.isFunction(callbacks.then)) {
						...
						return capstone.populateRelated(results, callbacks.then, next);
					}
				}
			}
...
View.prototype.render = function (renderFn, locals, callback) {
	...
	if (typeof renderFn !== 'function') {
		throw new Error('Capstone.View.render() renderFn must be a templatePath (string) or a function.');
	}
	...
	// Add Capstone's global pre('render') queue
	capstone.getMiddleware('pre:render').forEach(function (fn) {
	...
	async.eachSeries(this.initQueue, function (i, next) {
		if (Array.isArray(i)) {
			...
			throw new Error('Capstone.View.render() events must be functions.');
		}
	}, function (err) {
