...
```JS
var storage = new capstone.Storage({
	adapter: capstone.Storage.Adapters.FS,
	fs: {
		path: capstone.expandPath('./uploads'), // required; path where the files should be stored
...
- **retryAttempts**: _(number; default: 3)_ If `whenExists` is set to `'retry'`, how many times capstone should try to generate a unique filename before returning an error
- **publicPath**: _(string)_ Optional path the files will served from by the webserver

### Schema

The FS adapter supports all the default Capstone file schema fields. It also additionally supports and enables the `filename` path (required).
