...
> The new Storage API in CapstoneJS 0.4+ is replacing the `azureFile`, `localFile`, `localFiles` and `s3File` fields from CapstoneJS 0.3.x. If you use these field types in your project, you'll need to replace them with the new `File` field type and the appropriate storage adapter.

Often sites will need to store some files. In order to make this happen for a wide variety of filetypes and services, we've made a generic API (`capstone.Storage`) which is pluggable and can be integrated with different file backends via _adapters_. Adapters allow you to connect your storage instance to a specific service.
...
var myStorage = new capstone.Storage({
  ...
  fs: {
  	path: capstone.expandPath('./uploads'),
...
- FS: _(built-in)_ Use via `capstone.Storage.Adapters.FS`
- S3: Use via [`capstone-storage-adapter-s3`](http://npm.im/capstone-storage-adapter-s3) npm module
- Azure: Use via [`capstone-storage-adapter-azure`](http://npm.im/capstone-storage-adapter-azure) npm module
