...
Page.prototype.set = function (key, value) {
	...
	if (!key) {
		throw new Error('capstone.content.Page.set() Error: must be provided with a key to set a value.');
	}
...
Page.prototype.get = function (key) {
	if (!key) {
		throw new Error('capstone.content.Paget.get() Error: must be provided with a key to get a value.');
	}
...
Page.prototype.add = function (fields) {
	...
	if (!utils.isObject(fields)) {
		throw new Error('capstone.content.Page.add() Error: fields must be an object.');
	}
	...
	_.forEach(fields, function (options, path) {
		...
		if (typeof options.type !== 'function') {
			throw new Error('capstone.content.page.add() Error: Page fields must be specified with a type function');
		}

		if (options.type.prototype.__proto__ !== Type.prototype) { // eslint-disable-line no-proto

			// Convert native field types to their default Capstone counterpart

			if (options.type === String) {
				options.type = capstone.content.Types.Text;
			}
			...
			// 	options.type = Field.Types.Datetime;
				...
				throw new Error('capstone.content.page.add() Error: Unrecognised field constructor: ' + options.type);
			}
...
/**
 * Registers the page with Capstone.
 *
 * ####Example:
 *
 * 		var homePage = new capstone.content.Page('home');
 * 		// ...
 * 		homePage.register();
 *
 * 		// later...
 * 		var homePage = capstone.content.page('home');
 *
 * @api public
 */

Page.prototype.register = function () {
	return capstone.content.page(this.key, this);
};
