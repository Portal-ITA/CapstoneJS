 ...
 * ####Example:
 *
 *     capstone.content.fetch('home', function(err, content) { ... });
 *
 ...
 * ####Example:
 *
 *     capstone.content.store('home', { title: 'Welcome' }, function(err) { ... });
 *
 * @param {String} key
...
Content.prototype.page = function (key, page) {
	...
	if (arguments.length === 1) {
		...
		if (!this.pages[key]) {
			throw new Error('capstone.content.page() Error: page ' + key + ' cannot be registered more than once.');
		}
...
Content.prototype.initModel = function () {
	...
	var contentSchemaDef = {
		...
		data: { type: capstone.mongoose.Schema.Types.Mixed },
	};

	var ContentSchema = new capstone.mongoose.Schema(contentSchemaDef);

	var PageSchema = new capstone.mongoose.Schema({
	...
	this.AppContent = capstone.mongoose.model('App_Content', PageSchema);
...
Content.prototype.editable = function (user, options) {
	...
	if (!user || !user.canAccessCapstone) {
		return undefined;
	}

	if (options.list) {

		var list = capstone.list(options.list);
