...
var debug = require('debug')('capstone:core:list:register');


/**
 * Registers the Schema with Mongoose, and the List with Capstone
 *
...
function register () {
	...
	if (this.schema.methods.toCSV) {
		console.warn(this.key + ' Warning: List.schema.methods.toCSV support has been removed from CapstoneJS.\nPlease use getCSVData instead (see the 0.3 -> 4.0 Upgrade Guide)\n');
	}
	/* Apply Plugins */
	...
	if (this.get('inherits')) {
		...
		this.model = capstone.mongoose.model(this.key, this.schema);
	}
	...
	/* Register the list and its field types on the Capstone instance */
	capstone.lists[this.key] = this;
	capstone.paths[this.path] = this.key;
	assign(capstone.fieldTypes, this.fieldTypes);
