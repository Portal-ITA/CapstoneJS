...
// Update Schema - automatically created and managed by Capstone when updates are used
var UpdateModel = new mongoose.Schema(
	...
	{
		collection: capstone.prefixModel("App_Update")
	}
);
...
exports.apply = function(callback) {
	...
	var updatesPath = capstone.getPath("updates", "updates");
			...
			if (err) {
				...
				if (_.isObject(update.create)) {
					...
					update = function(done) {
						capstone.createItems(items, ops, function(err, stats) {
							if (!err) {
	...
	if (!fs.existsSync(updatesPath)) {
		console.log(
			"\nCapstoneJS Update Error:\n\n" +
				...
				"See http://v4.capstonejs.com/docs/configuration/#updates for more information.\n"
		);
		process.exit();
	}
	...
	async.eachSeries(updates, applyUpdate, function(err) {
		...
		if (err) {
			var errmsg =
				"An error occurred applying updates, bailing on Capstone init.\n\nError details:";
			if (!(updateCount || deferCount || skipCount)) {
				errmsg = _dashes_ + "\n" + errmsg;
			}
			logError(errmsg);
