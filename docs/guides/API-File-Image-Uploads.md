...
Image transfers to your CapstoneJS installation. It also shows how to
...
[CapstoneJS](https://github.com/capstonejs/capstone). All file path
...
they are all files to CapstoneJS. The difference is how you use them on
...
introduced in CapstoneJS v4.0 Beta. This code should be copied into the
...
var FileUpload = new capstone.List('FileUpload');

var myStorage = new capstone.Storage({
  adapter: capstone.Storage.Adapters.FS,
  fs: {
    path: capstone.expandPath('./public/uploads/files'), // required; path where the files should be stored
...
## Opening an API in CapstoneJS

Now we are going to create an API that can be used to upload and
download files to CapstoneJS. This is a two step process. The first step
...
exports = module.exports = function (app) {
  ...
  //File Upload Route
  app.get('/api/fileupload/list', capstone.middleware.api, routes.api.fileupload.list);
  app.get('/api/fileupload/:id', capstone.middleware.api, routes.api.fileupload.get);
  app.all('/api/fileupload/:id/update', capstone.middleware.api, routes.api.fileupload.update);
  app.all('/api/fileupload/create', capstone.middleware.api, routes.api.fileupload.create);
  app.get('/api/fileupload/:id/remove', capstone.middleware.api, routes.api.fileupload.remove);
  // ...TO HERE.
...
var FileData = capstone.list('FileUpload');
...
`public/fileAPITest.html`, start CapstoneJS, and open the test file in
your web browser. If CapstoneJS is already running you'll need to kill
the process and restart it. If CapstoneJS won't start after you create
    ...
    function uploadFile() {
      ...
      var opts = {
        ...
        success: function(data){
          //Dev Note: CapstoneAPI only allows file and image uploads with the file itself. Any extra metadata will have to
...
extension for CapstoneJS that mimicks the WordPress user interface.

The latest version of this file can be found in the
[christroutner/capstone-guides
repository](https://github.com/christroutner/capstone-guides).
