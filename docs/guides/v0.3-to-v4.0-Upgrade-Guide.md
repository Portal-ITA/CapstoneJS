...
Capstone Admin UI to a nonstandard express app (i.e. not using
`Capstone.start()`, but should make things simpler going forward.

### Changed

* `capstone.get('express session')` has been replaced by
`capstone.expressSession`
...
restricted routes)
* `capstone.routes(fn)` has been replaced by `capstone.set('routes',
fn)`
* the `routes` option supports both:
  * the legacy `function (app) {}` way of adding routes to the root
  express app
  * an `express.Router` instance
* `capstone.set('pre:routes')` now runs the hook as middleware before each
route is called, rather than running the function before Express starts up.

### Removed

* `capstone.static` has been removed (replacement TBA, see
`admin/server`)
* `capstone.mount` has been removed (replaced by more granular methods,
see `admin/server`)
* `capstone.bindEmailTestRoutes` and `capstone.set('email tests')`
support (was this actually being used by anyone? let me know -
@jedwatson)
* `capstone.set('email rules')` option has been removed; you should just
...
Spec](https://github.com/capstonejs/capstone/blob/master/admin/server/api/Readme.md))
...
- [The Storage API](https://github.com/capstonejs/capstone/tree/master/lib/storage)
- [Local FileSystem Adapter](https://github.com/capstonejs/capstone/blob/master/lib/storage/adapters/fs)
- [Azure Storage Adapter](https://github.com/capstonejs/capstone-storage-adapter-azure)
- [S3 Storage Adapter](https://github.com/capstonejs/capstone-storage-adapter-s3)

For the full upgrade guide for File fields, see
[File-Fields-Upgrade-Guide](https://github.com/capstonejs/capstone/wiki/File-Fields-Upgrade-Guide)
in the CapstoneJS Wiki.
...
When the `cloudinary folders` Capstone option is set, in 0.3.x
...
`Capstone.Email` was completely rewritten as the standalone, well-tested
...
* `capstone.set('email')` has been deprecated in favor of the `root`
option (passed to the `.Email` call)
* The `templateExt` option was deprecated in favor of the `engine`
option
* `capstone.set('transport')` has been deprecated in favor of setting
`transport` in the `new Email()` options
* `capstone.set('mailgun api key')` and `capstone.set('mailgun domain')`
have been deprecated in favor of passing the `apiKey` and `domain`
option to the send options in `.send`

## CSV Download

Capstone 0.3 had a feature where you could specify a custom `toCSV`
...
The `csv expanded` Capstone option has been deprecated. Relationship
...
Capstone 3 included `jade` as a dependency; this meant your project may
have been able to use jade as the view engine without depending on it
explicitly. Jade has now been removed from Capstone, so if you're using
it you will need to explicitly add it to your `package.json`.

### Underscore

Capstone 3 included `underscore` as a dependency; this was utilised by
...
If you get the following error when starting a project with Capstone 4:
