...
By default, Boostrap is already included with Capstone, so we would only have to add a few .less files. Copy contents of Boostrap theme's `less` folder, go to `public/styles/styles/site` and replace existing files with theme's files. Do not forget to `@import` correct .less files in `site.less`. Now re-run `node capstone` to recompile css.
...
As the last step, specify which fields to display in `capstone/pages` in Admin UI where all your pages will be listed. In our case we only want to display page title and state, and we set column width to 20%. `Page.defaultColumns = 'title, state|20%';` Now run `node capstone` and go to `http://localhost:3000/capstone/pages` to see our brand new `Page` model in action. Now create a new page called 'About'.
...
Last but not least, you may want to add `Pages` to Admin UI top navigation for easy access. To do so, open `capstone.js` file located in the root of your project and add a new route to `capstone.set(nav)`
...
Re-run `node capstone` and celebrate your new blog ready to go live.
...
This automatically created another config variable called `MONGODB_URI`. However, Capstone needs it to be called `MONGOLAB_URI`. To fix this, go to your project in your Heroku [dashboard](https://dashboard.heroku.com/), open the Settings tab and rename the variable to `MONGOLAB_URI`
