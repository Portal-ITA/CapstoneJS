# How to send emails with Capstone

We often had to send emails in our Capstone projects, so we figured we
...
That's everything there is to it from the Capstone side!
...
html(lang="en")
  head
  body
    h1 Your first email sent with Capstone!
    p Congratulations on sending your first email with Capstone
```
...
told Capstone where to send the email to yet!)
...
new Email('test-email.pug', {
  ...
  subject: 'Your first CapstoneJS email',
});
```
...
html(lang="en")
  head
  body
    h1 Hi #{ recipient.firstName } this is your second email sent with Capstone!
...
```sh
Hi Max, this is your second email sent with Capstone!
