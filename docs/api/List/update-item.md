...
A method to update an item that will apply Capstone's internal validators before attempting to update, to ensure data integrity.
...
Allows passing in of a mongoose object of a Capstone user. If the list has `tracking: true`, passing in a user will let `createdBy` or `updatedBy` to be set.
