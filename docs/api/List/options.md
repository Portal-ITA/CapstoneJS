...
The following is a list of all options for the Capstone List [constructor](/api/list), and their effects on the created list. Note that many of these can also be set by the constructor when the list is created.
...
*Warning: do not modify the `id` or `_id` schema options; the default behaviour is required by Capstone*

> NOTE If you're wondering how to control which navigation area Lists are categorised under in the Admin UI, check out the nav option in the CapstoneJS Configuration docs.
...
Disable creation of new items in the admin UI. Items may still be created by the Capstone server.

<h4 data-primitive-type="Boolean"><code>nodelete</code></h4>

Disable deleting items in the admin UI. Items may still be deleted by the Capstone server.

<h4 data-primitive-type="Boolean"><code>hidden</code></h4>

If true, the list is hidden from the Capstone admin UI.
...
Sets the number of items to be loaded per page on the Capstone admin UI.
...
Defines the default columns to be displayed in the Capstone admin UI list view. If nothing is provided, default columns will be set to just the name field.
...
This adds a hidden field to the schema which is `sortOrder`, which is a number. Capstone updates the `sortOrder` whenever an item is dragged in the admin UI, changing all necessary items to correct the sortOrder.
...
You can make a list that inherits from another registered Capstone list. This can add new fields to a schema, as well as adding a `__t` property to the schema with the value of the sublist name. In mongo, only one collection is created.
...
Post.register();
var ReferencedArticle = new capstone.List('ReferencedArticle', { inherits: Post });
...
In the Capstone admin UI, both the list and the sub-list will be visible.
