...
Capstone is designed to streamline running an [ExpressJS](https://expressjs.com) application including configuration of common options.

In this tutorial section we are going to add *routing* to your Capstone application. Routing is the process of directing client requests (from web browsers or API calls) to corresponding handling functions in your application.
...
Capstone should already be installed and at a minimum your `capstone.js` file should include the following:
...
If you completed [Part 2: Data Model Setup](/getting-started/setting-up/part-2) your `capstone.js` will have additional database configuration options. The database configuration options aren't required for the basic routing and web page set up in this tutorial section, but you'll need to keep those details for the subsequent tutorial section ([Part 4: Adding data from a form](/getting-started/setting-up/part-4)).

## Adding a new page view

### Modifying the `capstone.js` file

As mentioned in Part 1, `capstone.init` defines initial options for Capstone's startup. For rendering application views we are going to add two new properties to  `capstone.init`, and then add a line that will import our routes.

The two properties to add are:
 1. `views`: A folder location relative to `capstone.js` to load view files from. We're going to use `'templates/views'`.
 2. `view engine`: A template engine for Capstone to use to render view files. This will be `pug`.
...
Capstone will look for an installed `npm` package with the same name as the view engine, so you are going to have to install `pug` to get this working.

```sh
$ npm install --save pug
```

Finally, add a line to tell Capstone where you plan to define your routes:
...
Next, we need to add `routes` files. We are going to construct them using Capstone's recommended file and directory layout, however you can structure  differently if you prefer.
...
This file is going to export a function, take in the Express app Capstone has built for us, and add on our individual routes.
...
The Capstone importer gives us a function that allows us to reduce a folder and its contents to an object with the same nesting.
...
html(lang="en")
  ...
  body
    h1 Welcome to Capstone!
    #container.col
      p.
          Hope you're enjoying learning about CapstoneJS. We're close to some very dynamic cool things.
```

Now, if you start your Capstone app using `node capstone` you should be able to visit the homepage and see it rendered!
...
- [capstone.set](/api/methods/set)
- [init options](/documentation/configuration)
- [capstone.importer](/api/methods/importer)
