...
The path to your application's **view templates**. This is required for using the `capstone.View` Class, and will also be set on the express app.
...
If you want Capstone to automatically compile **.less** files into **.css** files, set this value to the same path as the `static` option.
...
If you want Capstone to automatically compile **.sass** files into **.css** files, set this value to the same path as the `static` option.
...
> Note that enabling this option requires you to have specified the `node-sass` package as a dependency in your project's `package.json` file; it is not automatically included with Capstone.
...
capstone.start({
  onHttpServerCreated: function() {
    var server = capstone.httpServer;
  }
});
```
...
There are two ways to implement HTTPS for your CapstoneJS application: either use a web server (e.g. [NGINX](https://nginx.com)) or PAAS (e.g. [Heroku](https://heroku.com)) that handles it for you, or set the following options to use the [https server provided by node.js](https://nodejs.org/api/https.html).
...
When set to "only", only the https server will be started. If `ssl key` or `ssl cert` are invalid, CapstoneJS will not start.
...
Exposes `onHttpsServerCreated` event during `capstone.start()`

> WARNING: If you intend to enable SSL on your CapstoneJS app, make sure you're using Node.js `0.10.33` or newer. Node versions prior to `0.10.33` are susceptible to the POODLE (Padding Oracle On Downgraded Legacy Encryption) vulnerability, a man-in-the-middle attack that targets `SSLv3` (see [CVE-2014-3566](https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2014-3566)). As of Node version `0.10.33`, the `SSLv2` and `SSLv3` protocols are disabled by default. For more information see the release notes for [Node v0.10.33 (Stable)](https://nodejs.org/en/blog/release/v0.10.33/).
...
Exposes `onSocketServerCreated` event during `capstone.start()`
