...
The following options control the branding, navigation and default export settings of the CapstoneJS application in the Admin UI:

<h4 data-primitive-type="String"><code>name</code></h4>

The name of the CapstoneJS application
...
This setting tells Capstone the root path of your app. By default, `module root` points to the path of the first script that required Capstone within your app. This default may be undesirable at times.
...
`module root` is used by Capstone's `.getPath()` method to resolve/expand the paths of the `views`, `favicon`, `extensions`, `ssl cert`, `ssl key`, `ssl ca`, `emails`, and `updates` settings.
...
This settings tells Capstone how to handle `iframe` tags. It does this by setting the response `X-Frame-Options` header. This header is used to protect against "ClickJacking" attacks.
...
> NOTE
> The `app` and `mongoose` options replace the functionality of the `capstone.connect()` method which is now deprecated. Due to changes in Express 4, `capstone.connect()` no longer works as expected.
