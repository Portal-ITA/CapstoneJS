...
Capstone includes an updates framework, which you can enable by setting the `auto update` option to `true`.
...
const PostCategory = capstone.list('PostCategory');
const Post = capstone.list('Post');

const importData = [
	{ name: 'A draft post', category: 'Capstone JS' },
	...
];
...
Fixing this properly would require significant refactoring of how the update framework works, which we're unable to commit to for the moment. [The relevant GitHub Issue is here](https://github.com/capstonejs/capstone/issues/2980), and of course PRs are always welcome.
