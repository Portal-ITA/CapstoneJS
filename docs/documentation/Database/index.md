...
Warn: CapstoneJS requires MongoDB v2.4 or greater.

In CapstoneJS, your data schema and models are controlled by **Lists**, and documents in your database are often called **Items**.

To define a data model, you create a `new capstone.List`, and pass it [list options](/documentation/database/#options).

You then `add` fields to the list. Behind the scenes, a Capstone List will create a [mongoose schema](http://mongoosejs.com/docs/guide.html), and add the appropriate paths to it for the fields you define.

The `schema` is accessible, allowing you to plug in other mongoose functionality like virtuals, methods and pre / post hooks.

When you have finished setting up your List, call `list.register()` to initialise it and register it with Capstone.
...
`new capstone.List(key:string [, options:object]);`

The syntax for creating a **Capstone List** is very similar to the syntax for creating a Mongoose Schema, with the exception of the constructor, which is `var MyList = new capstone.List(key, options)`.
...
When all the fields and options have been set on the list, call `MyList.register()` to register the list with Capstone and finalise its configuration.
...
var BasePage = new capstone.List('BasePage', {
  map: { name: 'title' },
  autokey: { path: 'slug', from: 'title', unique: true },
});
BasePage.add({
...
var ChildPage = new capstone.List('ChildPage', { inherits: BasePage });
...
Person.add(
  ...
  'Permissions',
  { isAdmin: { type: Boolean, label: 'Can access Capstone', index: true } },
  // header object
  { heading: 'Activities' },
...
When adding `fields` to `Lists`, you can either specify basic data types or Capstone Field Types.

### Overview

**Capstone Fields** allow you to easily add rich, functional fields to your application's models. They are designed to describe not just the structure of your data, but also the intention of your data. They provide:
- Rich controls in Capstone's Admin UI
...
Basic data types are mapped to their corresponding Capstone field types:
