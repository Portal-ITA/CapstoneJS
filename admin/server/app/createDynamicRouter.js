...
module.exports = function createDynamicRouter (capstone) {
	// ensure capstone nav has been initialised
	// TODO: move this elsewhere (on demand generation, or client-side?)
	if (!capstone.nav) {
		capstone.nav = capstone.initNav();
	}
    ...
    var LanguageRoute = require('../routes/language');
	...
	// Bind the request to the capstone instance
	router.use(function (req, res, next) {
		req.capstone = capstone;
		next();
	});

	// Pre adminroutes middleware
	if (typeof capstone.get('pre:adminroutes') === 'function') {
		capstone.get('pre:adminroutes')(router);
	}
	router.use(function (req, res, next) {
		capstone.callHook('pre:adminroutes', req, res, next);
	});

	if (capstone.get('healthchecks')) {
		router.use('/server-health', require('./createHealthchecksHandler')(capstone));
	}
	...
	// #1: Session API
	// TODO: this should respect capstone auth options
	...
	if (capstone.get('auth') === true) {
		// TODO: poor separation of concerns; settings should be defaulted elsewhere
		if (!capstone.get('signout url')) {
			capstone.set('signout url', '/' + capstone.get('admin path') + '/signout');
		}
		if (!capstone.get('signin url')) {
			capstone.set('signin url', '/' + capstone.get('admin path') + '/signin');
		}
		if (!capstone.nativeApp || !capstone.get('session')) {
			router.all('*', capstone.session.persist);
		}
		router.all('/signin', SigninRoute);
        router.all('/signout', SignoutRoute);
        router.all('/language/:lang', LanguageRoute);
		router.use(capstone.session.capstoneAuth);
	} else if (typeof capstone.get('auth') === 'function') {
		router.use(capstone.get('auth'));
	}
