...
module.exports = function createStaticRouter (capstone) {
	var capstoneHash = capstone.createCapstoneHash();
	var writeToDisk = capstone.get('cache admin bundles');
	var router = express.Router();

	/* Prepare browserify bundles */
	var bundles = {
		fields: browserify({
			stream: buildFieldTypesStream(capstone.fieldTypes),
			expose: 'FieldTypes',
			file: './FieldTypes.js',
			hash: capstoneHash,
	...
	// prebuild static resources on the next tick in capstone dev mode; this
	// improves first-request performance but delays server start
	if (process.env.CAPSTONE_DEV === 'true' || process.env.CAPSTONE_PREBUILD_ADMIN === 'true') {
	...
	var capstoneTinymcePath = path.dirname(require.resolve('capstone-tinymce'));
	var customStylesPath = capstone.getPath('adminui custom styles') || '';
	...
	var lessOptions = {
		render: {
			...
			modifyVars: {
				...
				capstoneTinymcePath: JSON.stringify(capstoneTinymcePath),
				customStylesPath: JSON.stringify(customStylesPath),
				adminPath: JSON.stringify(capstone.get('admin path')),
			},
		},
	};
	...
	router.use('/styles/fonts', express.static(`${capstoneTinymcePath}/skin/fonts`));
	router.get('/js/fields.js', bundles.fields.serve);
	router.get('/js/signin.js', bundles.signin.serve);
	router.get('/js/admin.js', bundles.admin.serve);
	router.use('/js/lib/tinymce/skins/capstone', express.static(`${capstoneTinymcePath}/skin`));
	router.use('/js/lib/tinymce/plugins/uploadimage', express.static(`${capstoneTinymcePath}/plugins/uploadimage`));
