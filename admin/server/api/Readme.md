...
The plan is to move this API Functionality into a lower level in Capstone's core so that they are available through official Capstone APIs, and the Admin APIs become default implementations of the generic features.
...
Manages creating, destroying and validating CapstoneJS sessions (specifically, logged in users)
...
{
	name: 'Test User',
	email: 'user-1469454400942@capstonejs.com',
	password: 'test1234'
}
```
...
{
	'name.full': 'Test User',
	email: 'user@capstonejs.com', // already exists in the database
	password: 'test1234',
...
// 500 database error
{
	...
	detail: {
		code: 11000
		index: 0
		errmsg: 'E11000 duplicate key error index: capstone-test.users.$email_1 dup key: { "user@capstonejs.com" }',
		op: {
			password: '******',
			email: 'user@capstonejs.com',
			isProtected: false,
