	...
	query.exec(function (err, results) {
		...
		var sendCSV = function (data) {
			...
			var content = baby.unparse(data, {
				delimiter: capstone.get('csv field delimiter') || ',',
			});
			...
			if (_.last(deps) === 'callback') {
				...
				return async.map(results, function (i, callback) {
					...
					if (err) {
						...
						console.log(err);
						return res.send(capstone.wrapHTMLError('Error generating CSV', 'Please check the log for more details, or contact support.'));
					}
					sendCSV(results);
				});
			} else {
