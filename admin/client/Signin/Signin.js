...
var SigninView = React.createClass({
	...
	handleSubmit(e) {
		...
		xhr(
			{
				url: `${Capstone.adminPath}/api/session/signin`,
				method: "post",
				json: {
			...
			(err, resp, body) => {
				if (err || (body && body.error)) {
					...
					if (Capstone.redirect) {
						top.location.href = Capstone.redirect;
					} else {
			...
			<div className="auth-wrapper">
				...
				<div className={boxClassname}>
					<h1 className="u-hidden-visually">
						{this.props.brand ? this.props.brand : "Capstone"} Sign In{" "}
					</h1>
					<div className="auth-box__inner">
						...
						{this.props.user ? (
							<UserInfo
								adminPath={
									this.props.from ? this.props.from : Capstone.adminPath
								}
								signoutPath={`${Capstone.adminPath}/signout`}
								usercanAccessCapstone={this.props.usercanAccessCapstone}
