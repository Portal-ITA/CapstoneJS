...
const ListView = React.createClass({
	...
	componentWillMount () {
		...
		this.setState({
			showCreateForm: (shouldOpenCreate && !isNoCreate) || Capstone.createFormErrors,
		});
	...
	onCreate (item) {
		...
		this.context.router.push(`${Capstone.adminPath}/${list.path}/${item.id}`);
	},
	createAutocreate () {
		const list = this.props.currentList;
		list.createItem(null, (err, data) => {
			if (err) {
				...
				this.context.router.push(`${Capstone.adminPath}/${list.path}/${data.id}`);
			}
		});
	},
	updateSearch (e) {
