...
export const EditFormHeader = React.createClass({
	...
	renderDrilldownItems () {
		...
		let backPath = `${Capstone.adminPath}/${list.path}`;
	...
	renderSearch () {
		...
		return (
			<form action={`${Capstone.adminPath}/${list.path}`} className="EditForm__header__search">
	...
	renderCreateButton () {
		...
		let props = {};
		if (autocreate) {
			props.href = '?new' + Capstone.csrf.query;
		} else {
