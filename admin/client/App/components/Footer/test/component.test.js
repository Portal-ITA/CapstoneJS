...
describe('<Footer />', () => {
	...
	it('should have a data prop of "capstone-footer"', () => {
		const component = shallow(<Footer />);
		demand(component.prop('data-capstone-footer')).eql(true);
	});

	// TODO FIX THIS, possibly a test problem
	it.skip('should render a link to CapstoneJS', () => {
		const component = shallow(
			<Footer />
		);
		demand(component.find('a[href="http://capstonejs.com"]').length).eql(1);
	});
	...
	describe('this.props', () => {
		...
		it('should render the CapstoneJS version', () => {
			const version = '1.0';
			const component = shallow(
				<Footer
					version={version}
