...
function cloudinaryimage (list, path, options) {
	...
	if (!capstone.get('cloudinary config')) {
		throw new Error(
			...
			+ 'See http://v4.capstonejs.com/docs/configuration/#services-cloudinary for more information.\n'
		);
	}
}
...
cloudinaryimage.prototype.getFolder = function () {
	var folder = null;
	if (capstone.get('cloudinary folders') || this.options.folder) {
		if (typeof this.options.folder === 'string') {
			folder = this.options.folder;
		} else {
			var folderList = capstone.get('cloudinary prefix') ? [capstone.get('cloudinary prefix')] : [];
...
cloudinaryimage.prototype.addToSchema = function (schema) {
	...
	var src = function (item, options) {
		...
		if (!('fetch_format' in options) && capstone.get('cloudinary webp') !== false) {
			options.fetch_format = 'auto';
		}
		if (!('progressive' in options) && capstone.get('cloudinary progressive') !== false) {
			options.progressive = true;
		}
		if (!('secure' in options) && capstone.get('cloudinary secure')) {
			options.secure = true;
		}
...
cloudinaryimage.prototype.getOptions = function () {
	...
	// to capstone after the model is registered, it will still be respected.
	// Setting secure overrides default `cloudinary secure`
	if ('secure' in this.options) {
		this.__options.secure = this.options.secure;
	} else if (capstone.get('cloudinary secure')) {
		this.__options.secure = capstone.get('cloudinary secure');
	}
...
cloudinaryimage.prototype.updateItem = function (item, data, files, callback) {
	...
	if (uploadedFile) {
		var tagPrefix = capstone.get('cloudinary prefix') || '';
		var uploadOptions = {
			tags: [],
		};
		if (tagPrefix.length) {
		...
		if (capstone.get('env') !== 'production') {
...
cloudinaryimage.prototype.getRequestHandler = function (item, req, paths, callback) {
	...
	return function () {
		...
		if (req.body && req.body[paths.select]) {
			...
			var tp = capstone.get('cloudinary prefix') || '';
			var imageDelete;
			if (tp.length) {
				tp += '_';
			}
			...
			if (capstone.get('cloudinary folders')) {
				uploadOptions.folder = item.get(paths.folder);
			}
			if (capstone.get('cloudinary prefix')) {
				uploadOptions.tags.push(capstone.get('cloudinary prefix'));
			}
			if (capstone.get('env') !== 'production') {
