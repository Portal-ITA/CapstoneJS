...
exports.testFieldType = function (List) {
	...
	describe('format', function () {
		it('should strip the protocol when formatting', function (done) {
			var testItem = new List.model();
			List.fields.url.updateItem(testItem, {
				url: 'http://www.capstonejs.com',
			}, function () {
				demand(testItem._.url.format()).be('www.capstonejs.com');
				done();
			});
		});
		...
		it('should call custom format methods', function (done) {
			...
			List.fields.customFormat.updateItem(testItem, {
				customFormat: 'http://www.capstonejs.com',
			}, function () {
				demand(testItem._.customFormat.format()).be('HTTP://WWW.CAPSTONEJS.COM');
				done();
			});
		});
	});
