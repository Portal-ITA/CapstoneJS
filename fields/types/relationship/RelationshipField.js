...
module.exports = Field.create({
	...
	buildFilters () {
		...
		_.forEach(this.props.filters, (value, key) => {
			if (typeof value === 'string' && value[0] === ':') {
				...
				if (fieldName === '_id' && Capstone.item) {
					filters[key] = Capstone.item.id;
					return;
				}
			} else {
				filters[key] = value;
	...
	cacheItem (item) {
		item.href = Capstone.adminPath + '/' + this.props.refList.path + '/' + item.id;
	...
	loadValue (values) {
		...
		async.map(values, (value, done) => {
			xhr({
				url: Capstone.adminPath + '/api/' + this.props.refList.path + '/' + value + '?basic',
				responseType: 'json',
			}, (err, resp, data) => {
	...
	loadOptions (input, callback) {
		...
		xhr({
			url: Capstone.adminPath + '/api/' + this.props.refList.path + '?basic&search=' + input + '&' + filters,
			responseType: 'json',
		}, (err, resp, data) => {
			if (err) {
