...
var RelationshipColumn = React.createClass({
	...
	renderMany (value) {
		...
		for (let i = 0; i < 3; i++) {
			...
			items.push(
				<ItemsTableValue interior truncate={false} key={'anchor' + i} to={Capstone.adminPath + '/' + refList.path + '/' + value[i].id}>
					{value[i].name}
				</ItemsTableValue>
			);
		}
	...
	renderValue (value) {
		...
		const refList = this.props.col.field.refList;
		return (
			<ItemsTableValue to={Capstone.adminPath + '/' + refList.path + '/' + value.id} padded interior field={this.props.col.type}>
				{value.name}
			</ItemsTableValue>
		);
	},
