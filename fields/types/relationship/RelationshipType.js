...
function relationship (list, path, options) {
	...
	this._nativeType = capstone.mongoose.Schema.Types.ObjectId;
...
relationship.prototype.addToSchema = function (schema) {
	...
	schema.virtual(this.paths.refList).get(function () {
		return capstone.list(field.options.ref);
	});
...
definePrototypeGetters(relationship, {
	...
	isValid: function () {
		return capstone.list(this.options.ref) ? true : false;
	},
	// Returns the Related List
	refList: function () {
		return capstone.list(this.options.ref);
	},
