...
var RelationshipFilter = React.createClass({
	...
	populateValue (value) {
		async.map(value, (id, next) => {
			...
			xhr({
				url: Capstone.adminPath + '/api/' + this.props.field.refList.path + '/' + id + '?basic',
				responseType: 'json',
			}, (err, resp, data) => {
	...
	loadSearchResults (thenPopulateValue) {
		...
		xhr({
			url: Capstone.adminPath + '/api/' + this.props.field.refList.path + '?basic&search=' + searchString + '&' + filters,
			responseType: 'json',
		}, (err, resp, data) => {
			if (err) {
				// TODO: Handle errors better
