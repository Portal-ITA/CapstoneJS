...
See https://github.com/capstonejs/capstone/wiki/File-Fields-Upgrade-Guide

TODO: this is used by capstone/admin/server/api/s3.js to generate headers, and should be factored out
*/
...
function s3file(list, path, options) {
	...
	if (!this.s3config) {
		throw new Error('Invalid Configuration\n\n'
			...
			+ 'See http://v4.capstonejs.com/docs/configuration/#services-amazons3 for more information.\n');
	}
...
/**
 * Exposes the custom or capstone s3 config settings
 */
Object.defineProperty(s3file.prototype, "s3config", {
	get: function() {
		return this.options.s3config || capstone.get("s3 config");
	}
});
