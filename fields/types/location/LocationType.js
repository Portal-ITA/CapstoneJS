...
function location (list, path, options) {
	...
	this.enableMapsAPI = (options.enableImprove === true || (options.enableImprove !== false && capstone.get('google server api key'))) ? true : false;
...
function doGoogleGeocodeRequest (address, region, callback) {
	...
	// Please make sure your Capstone app complies with the Google Maps API License.
	...
	if (capstone.get('google server api key')) {
		options.key = capstone.get('google server api key');
	}
...
location.prototype.googleLookup = function (item, region, update, callback) {
	...
	doGoogleGeocodeRequest(address, region || capstone.get('default region'), function (err, geocode) {
