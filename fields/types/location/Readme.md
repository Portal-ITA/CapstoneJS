...
Google Places integration requires the `google api key` option to be set for Capstone. See the [Google configuration documentation](http://v4.capstonejs.com/docs/configuration/#services-google) for details.
...
Options sets `enableMapsAPI` to true. If it is not set, `enableMapsAPI` is set to true if `google server api key` is set in capstone.
...
The Geocoding API may only be used in conjunction with a Google map; geocoding results without displaying them on a map is prohibited. Please make sure your Capstone app complies with the Google Maps API License.
