...
The Type is the part of the field users use in their Capstone models, e.g. `postTitle: { type: Types.Text }`. Each type should have four main functions:
...
Capstone lets users choose which data of their documents they want to display in the overview. Since we cannot display booleans, text, passwords, etc. all in the same way they each have a `Column` react component that dictates how they look there.
