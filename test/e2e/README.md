...
This is an overview of the end-2-end UI/functional testing for capstone.  UI/functional tests ensure
regression coverage of all aspects of a real capstone application.  The tests use a real capstone
...
for nightwatch specific structure as well as partly based on a typical capstone app since the e2e test runs
with a real capstone app server.

    test/e2e
        global.js                               => common nightwatch test environment config

        server.js                               => capstone test app server
        ...
        updates                                 => all schema update/migration files
           0.0.1-updates-e2e.js                 => capstone updates

        models                                  => all test list models
           User.js                              => capstone user list model
           ...


## Running
Testing is a critical part of any capstone commit to ensure the commit has not introduced any
UI or functional regressions.  Make sure to run all capstone tests prior to pushing any commits.
...
from capstone's root directory:
    ...
    Pre-requisites:
        ...
        - Make sure that port 3000 is available; if not please tell the e2e server what port it
          should bind to.  For example, to use port 9999 do the following (in a bash shell):
            ...
            export CAPSTONEJS_PORT=9999

        - Make sure to:

            export CAPSTONE_PREBUILD_ADMIN=true
    ...
    If you want to run the e2e capstone test app server standalone then run as follows:

        export CAPSTONEJS_PORT=9999 && node test/e2e/server.js --notest

    If you want to run the e2e capstone test app server standalone without dropping the database then run as follows:

        export CAPSTONEJS_PORT=9999 && node test/e2e/server.js --notest --nodrop
...
## Field Test Objects
In capstone we have come up with the concept of a Field Test Object.  These are similar to Nightwatch's Page Objects, 
...
## Model Test Configs
In capstone we have come up with the concept of a Model Test Config.  In a capstone application, a list model is used to
define the fields that make up the model.  Likewise, a Model Test Config is used to describe the Field Test Objects for
fields the capstone list model is composed of.  This is a powerful concept as you can also create Model Test Config for your
own application and plug into the capstone's test framework to test your app specific models!


## Testing Your Application Models
Capstone's e2e test framework can be used to test your own application's list models.
