...
describe('Capstone.session', function () {

	describe('capstone.session.signinWithUser()', function () {
		...
		before(function () {
			capstone.get('cookie secret', 'SECRET');
			capstone.set('user model', 'User');
		});
		...
		describe('with valid args, "cookie signin" on', function () {
			...
			it('should regenerate session, set user, session.userId, and res.cookie', function () {
				capstone.set('cookie signin', true);
				capstone.session.signinWithUser(user, req, res, onSuccess);
				...
				sinon.assert.calledWith(res.cookie, 'capstone.uid');
		...
		describe('with valid args, "cookie signin" off', function () {
			...
			it('should regenerate session, set user, session.userId', function () {
				capstone.set('cookie signin', false);
				capstone.session.signinWithUser(user, req, res, onSuccess);
		...
		describe('with invalid args', function () {
			it('should error when called less then 4 args', function () {
				function callWithNoArgs() {
					capstone.session.signinWithUser();
				}
				callWithNoArgs.must.throw('capstone.session.signinWithUser requires user, req and res objects, and an onSuccess callback.');

				function callWithOneArg() {
					capstone.session.signinWithUser(user);
				}
				callWithOneArg.must.throw('capstone.session.signinWithUser requires user, req and res objects, and an onSuccess callback.');

				function callWithTwoArgs() {
					capstone.session.signinWithUser(user, req);
				}
				callWithOneArg.must.throw('capstone.session.signinWithUser requires user, req and res objects, and an onSuccess callback.');

				function callWithThreeArgs() {
					capstone.session.signinWithUser(user, req, res);
				}
				callWithOneArg.must.throw('capstone.session.signinWithUser requires user, req and res objects, and an onSuccess callback.');
			});
			...
			it('should error when user arg is not an object', function () {
				...
				callWithInvalidUser.must.throw('capstone.session.signinWithUser requires user to be an object.');
			});
			...
			it('should error when req arg is not an object', function () {
				...
				callWithInvalidReq.must.throw('capstone.session.signinWithUser requires req to be an object.');
			});
			...
			it('should error when res arg is not an object', function () {
				...
				callWithInvalidRes.must.throw('capstone.session.signinWithUser requires res to be an object.');
			});
			...
			it('should error when onSuccess arg is not a function', function () {
				...
				callWithInvalidCallback.must.throw('capstone.session.signinWithUser requires onSuccess to be a function.');
			});
	...
	// TODO: need more test for capstone.session.signin()
	describe('capstone.session.signin()', function () {
		...
		describe('case-insensitive email lookup', function () {
			...
			before(function () {
				...
				capstone.set('user model', 'User');
				sinon.stub(capstone, 'list').withArgs('User').returns(this.User);
				sinon.stub(capstone.session, 'signinWithUser').callsArg(3);
			});

			after(function () {
				capstone.list.restore();
				capstone.session.signinWithUser.restore();
			});
			...
			it('should match email with mixed case', function (done) {
				...
				capstone.session.signin(lookup, null, null, function () {
					...
					sinon.assert.calledOnce(capstone.session.signinWithUser);
					done();
			...
			it('should not match email when invalid', function (done) {
				...
				capstone.session.signin(lookup, null, null, this.onSuccess, function (err) {
					...
					sinon.assert.notCalled(capstone.session.signinWithUser);
					done();
				}.bind(this));
			});
	...
	describe('capstone.session.signout()', function () {
		...
		it('should unset user, session.userId and cookie', function () {
			capstone.set('cookie signin', true);
			capstone.session.signinWithUser(user, req, res, function() {
				capstone.session.signout(req, res, function() {
					demand(req.user).be.null();
					demand(req.session.userId).be.null();

					sinon.assert.calledOnce(res.clearCookie);
					sinon.assert.calledWith(res.clearCookie, 'capstone.uid');
				});
			});
		});
	...
	// TODO: test capstone.session.persist()
	// describe('capstone.session.persist()');

	// TODO: test capstone.session.capstoneAuth()
	// describe('capstone.session.capstoneAuth()');
