...
module.exports = function bindErrorHandlers (capstone, app) {
	...
	var default404Handler = function (req, res) {
		...
		res.status(404).send(capstone.wrapHTMLError('Sorry, no page could be found at this address (404)'));
	};

	app.use(function (req, res, next) {
		var err404 = capstone.get('404');
		if (err404) {
			try {
	...
	var default500Handler = function (err, req, res, next) { // eslint-disable-line no-unused-vars
		...
		var msg = '';
		if (capstone.get('env') === 'development') {
			if (err instanceof Error) {
				if (err.type) {
					msg += '<h2>' + err.type + '</h2>';
				}
		...
		return res.status(500).send(capstone.wrapHTMLError('Sorry, an error occurred loading the page (500)', msg));
	};

	app.use(function (err, req, res, next) {
		var err500 = capstone.get('500');
		if (err500) {
			try {
